...
LINK DEL REPOSITORIO: https://github.com/G1LB3T0/R_Studio_LAB2.git
...
reglas_B <- apriori(trans, parameter = list(support = 0.15, confidence = 0.70, target = "rules", minlen=2))
...
trans_limpias <- as(datos_limpios, "transactions")

# Generar nuevas reglas buscando relaciones entre presupuesto, duración y éxito
reglas_limpias <- apriori(trans_limpias, parameter = list(support = 0.05, confidence = 0.60, target = "rules", minlen=2))
...
## 4.1 Utilice uno de los algoritmos mencionados en la sección de instrucciones, explique por qué lo eligió para usarlo
## en este conjunto de datos en particular.
```{r}
set.seed(123)

idx_unique <- !duplicated(datos_scaled)
datos_scaled_u <- datos_scaled[idx_unique, ]
km_u <- km
km_u$cluster <- km$cluster[idx_unique]

tsne_model <- Rtsne(datos_scaled_u,
                    dims = 2,
                    perplexity = 30,
                    max_iter = 1000,
                    verbose = TRUE)
...
tsne_df <- data.frame(
  Dim1 = tsne_model$Y[,1],
  Dim2 = tsne_model$Y[,2],
  Cluster = as.factor(km_u$cluster)
)

ggplot(tsne_df, aes(x = Dim1, y = Dim2, color = Cluster)) +
  geom_point(alpha = 0.6) +
  theme_minimal() +
  labs(title = "Visualización t-SNE de las películas",
       x = "Dimensión 1",
       y = "Dimensión 2")
```
Se seleccionó el algoritmo t-SNE debido a su capacidad para capturar estructuras no lineales en datos de alta dimensión. A diferencia del PCA, que maximiza varianza bajo supuestos lineales, t-SNE preserva relaciones locales entre observaciones, lo que permite identificar agrupaciones complejas que podrían no ser visibles mediante técnicas lineales.

Se observa una gran masa central (Cluster 5) que concentra la mayoría de las películas. Esto indica que existe un segmento dominante del mercado cinematográfico con características financieras y de recepción similares, lo que confirma que gran parte de la industria se mueve en rangos económicos intermedios.

## 4.2. Interprete los resultados obtenidos y explique la relevancia o no relevancia de los hallazgos.

Al analizar la proyección generada por el algoritmo t-SNE, se validan y profundizan los hallazgos que obtuvimos previamente con K-Medias y PCA. La relevancia de este gráfico radica en que logra mapear las relaciones no lineales entre las películas, mostrando agrupaciones complejas y topologías que no eran tan evidentes mediante las técnicas lineales.

Lo que más resalta a simple vista es la enorme masa central consolidada, dominada casi en su totalidad por el Clúster 5 (los puntos azules). La relevancia de este hallazgo es que demuestra de forma gráfica cómo la inmensa mayoría de las producciones comparten un comportamiento financiero y de recepción sumamente similar. Al ser un bloque tan denso, nos confirma que el mercado cinematográfico estándar está bastante saturado; es un entorno donde es sumamente difícil que una cinta promedio logre diferenciarse visual o estadísticamente del resto de su competencia.

Al observar la periferia del gráfico, vemos que clústeres como el 2 (oliva) y el 6 (magenta) tienen un comportamiento muy disperso, formando arcos inconexos alrededor de la masa principal. Esta falta de cohesión espacial es un hallazgo clave porque confirma empíricamente que estos grupos no siguen un patrón de mercado estructurado. Al tratarse de las anomalías estadísticas y proyectos atípicos detectados antes, quedan representados gráficamente como ruido, indicándole a Cine Vision Studios que sería un error basar proyecciones de inversión en un perfil tan errático.

En contraste, el Clúster 4 (color turquesa) se proyecta como un bloque bastante sólido, denso y claramente desplazado hacia un nicho propio, adyacente pero independiente del aglomeramiento central. Esto tiene muchísima relevancia estratégica para el estudio. Nos permite visualizar de forma clara ese segmento de películas de presupuesto medio y alta rentabilidad. Comprueba gráficamente que sí existe un ecosistema de éxito comercial estable que logra aislarse de la masa promedio, ofreciendo un modelo de negocio mucho más seguro y alejado del riesgo que representan las periferias dispersas.
